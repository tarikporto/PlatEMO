...
for n = 1:length(problem)
    ...
    for nsp_config_index = 1:length(nsp_configs)
        ...
        for r = 1:runs
            main('-algorithm',{@NSGAIIDSD,nsp_config},'-problem',problem{n}, '-evaluation', 60000, '-save', 1)
    ...
    csvwrite(strcat('CSV/NSGAII_',problem_name,'_hv.csv'), HV_all);
    csvwrite(strcat('CSV/NSGAII_',problem_name,'_igd.csv'), IGD_all);
    csvwrite(strcat('CSV/NSGAII_',problem_name,'_mst.csv'), kruskal_all);
    csvwrite(strcat('CSV/NSGAII_',problem_name,'_shir.csv'), shir_all);
end